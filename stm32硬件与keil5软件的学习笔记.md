...
常用于调试、唤醒、测试等场景。
  >注: 写入后**必须手动清除~后文讲的~请求挂起寄存器（写 1 清零）**，否则会重复触发。

  .
- **请求挂起寄存器**（PR - Pending Register）
记录 EXTI 线当前有效的中断请求。
当边沿检测电路检测到有效边沿，并且触发方式已使能时，该位被置 1。
...
接收来自`请求挂起寄存器`的事件信号。
如果 EMR 允许事件，则生成一个短暂脉冲（通常是 1 个时钟周期）。
...
- **更新中断**:
  ...
  我们只要再设置好NVIC 的定时器通道, 就可以实现定时器的中断功能了.
  .
-  **定时器的中断/事件切换**:
      在刚刚的**定时中断**部分也讲了, 既有更新中断,也有更新事件,他们中间该如何切换呢?
      定时器的中断/事件切换与[EXTI内部框架](#内部框架)的原理相近:
      首先的,CNT计数器达到ARR寄存器设定的目标值后, 会自动清零,并且令`TIMx_SR`寄存器的相应位置1;
      然后在程序内设置相应中断/事件屏蔽寄存器;
      最后实现中断或事件;
      .   
- **主模式触发DAC**:
