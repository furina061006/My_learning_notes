...
由于上学后时间不大充裕, 在STM32片上外设的`SPI`和外接外设硬件的`W25Q64`往后的内容可能会略有考虑不周, 望海涵🙏🙏🙏.
## 目录
  ...
  - [I2C 集成电路间总线](#i2c-集成电路间总线)
    ...
    - [I2C 外设模块](#i2c-外设模块)
      ...
      - [时序图](#时序图)
        - [主发送器时序图](#主发送器时序图)
        - [主接收器时序图](#主接收器时序图)
    - [软件 / 硬件 通信波形对比](#软件--硬件-通信波形对比)
  - [SPI 串行外设接口](#spi-串行外设接口)
    - [SPI通信原理](#spi通信原理)
      - [简介](#简介-6)
      - [通信原理](#通信原理)
      - [基本时序](#基本时序)
    - [SPI外设模块](#spi外设模块)
      ...
      - [硬件SPI框图](#硬件spi框图)
      - [传输时序图](#传输时序图)
        - [全双工连续传输时序图](#全双工连续传输时序图)
        - [全双工非连续传输时序图](#全双工非连续传输时序图)
  - [软件使用](#软件使用)
    - [Keil\_v5](#keil_v5)
...
- 较串口而言的特点
  ...
  - **总线挂载多设备**:
    ...
    > 多主多从模式下, 又分为`固定多主机` 和 `可变多主机`.
    顾名思义, `固定多主机`就是, 主机和从机的角色固定;
    `可变多主机`就是, 设备都是从机, 但每个从设备在总线空闲时都可以跳出来作为主机发布号令, 随后再变回从机. 
...
#### I2C时序基本单元
以下针对[MPU6050](#mpu6050)的I2C通信协议进行举例.
> I2C协议只规定 起始条件后是寻址, 并没有规定后续数据位的用途, **后续数据位的用途是各个外设厂商规定的.**
> 毕竟I2C通信是活的, 硬件的排线是死的.
...
### I2C 外设模块
#### 简介
STM32内部集成了硬件I2C收发电路，可以由**硬件自动**执行**时钟生成**、**起始终止条件生成、应答位收发、数据收发**等功能，节约软件资源, 减轻CPU的负担
- 设计指标很高, 可以满足几乎所有的I2C协议的规定.
  .

- 支持`一主多从`, `多主多从`模型
   实际上, STM32内置的I2C 外设模块是按着可变多主机的模型设计的. 刚好也可以实现一主多从而已.
   所以在使用该模块时, 要**按谁要做主机, 谁就跳出来的思路操作**.
 
  .
- **支持7位/10位地址模式**
   当从设备太多, 地址不够分配的情况下, 我们可以选择开辟多条I2C总线;
   当然也可以选择使用10位地址模式.
   想要使用10位地址模式, 需要做以下几件事:
   1. 先在程序初始化I2C外设时, 配置寄存器,让外设知道要选择10位的地址模式.
   2. 由于10位地址的原因, 需要发送 2个字节的数据, 格式如下:
      1. 发送 `1111 0 + XX(地址最高两位) + Y(读写位)`
      2. 接收 `Ack` 
      <span style = "color : red">~所有支持~ ~10~ ~位模式的从机都会监听~ ~11110XXY~ ~这个前导码。如果~ ~XX~ ~匹配自己的高~ ~2~ ~位，从机立即发送~ ~ACK。~</span>
      3. 发送 `剩下的 8位 地址`
      4. 接收 `Ack`.


- 支持不同的通讯速度
  标准速度(高达100 kHz) / 快速(高达400 kHz)
  - 为什么最高速度只有400 kHz呢？
    因为I2C通信强下拉和弱上拉的通信方式, 导致上升沿耗费的时间比较长, 也就是从稳定的低电平到稳定的高电平耗时较长, 这一定程度上影响了通信速度.
    . 
  - 为什么要划分出两种速度呢?
    100 kHz是一个分界线. 
    当通讯速度 <= 100 kHz时, SCL时钟占空比为 50%;
    当通讯速度 > 100 kHz时, SCL时钟占空比 就需要通过配置寄存器选择 高低电平比 为 9 : 16 / 1 : 2 
    > **写入的数据还需要一定时间来保证其稳定**, 但是读, 只要一下就好, 所以低电平时间会分多一点.
    .    
- 支持DMA
- 兼容`SMBus`协议
  `SMBus`是 系统管理总线, `System Management Bus`.
> STM32F103C8T6 硬件I2C资源：I2C1、I2C2;
> 这意味着, STM32的I2C总线最多只能开两条.
  但**软件模拟I2C是可以无数条的**, 只要引脚够用()

#### 功能框图
![](./images/2026-02-26-02-36-02.png)
- `SDA` / `SCL`
  这两条通信线没有专属的露出引脚, 需要**通过复用 / 重映射 固定对应的GPIO引脚**与外界相连.
  .
- **数据寄存器 和 移位寄存器**
  这点和串口类似, 区别就是 I2C 这里读写共用一组.
  > 一样有 `TXE` 和 `RXNE`, 触发条件也一样.

  .
- **比较器和地址寄存器**
  STM32作为从机模式下使用, 和 从机里的地址寄存器一样.
  - 那双地址寄存器是什么??!
    它的核心作用是：让同一个 I2C 外设（同一个引脚）能够同时响应两个不同的 7 位从机地址。
    OAR2 最强大的地方在于它支持**地址掩码**。你可以配置它只匹配地址的高几位，低几位忽略. 这样你的OAR2 可以匹配一组的地址.
    孩子累了， 偷个懒：
    ![](./images/2026-02-26-02-52-13.png)
    > 关键区别：当通信结束后，STM32 会触发中断，并在状态寄存器（ISR 或 SR1）中告诉你：“刚才匹配的是 OAR1 还是 OAR2？”
    如果是 OAR1 匹配，你可能执行函数 `Handle_Address1()`。
    如果是 OAR2 匹配，你可能执行函数 `Handle_Address2()`。

- **帧错误校验 PEC**
  原理是[CRC检测](#crc检测).
  ```text
  [ 帧头 (2B) ] [ 命令 (1B) ] [ 长度 (1B) ] [ 数据 (N B) ] [ CRC8 (1B) ]
   0xEB 0x90        0x01           0x0A        (10 字节)      (??)
  ```
  在输出一帧的数据后, 自动对前面**数据位**进行CRC检测, 得到一个字节的校验位, 然后附加在后面.
  > 其他和串口的奇偶校验差不多, 都可以自行发送 / 接收校验位, 并在校验错误时, 置校验错误标志位.

  .
- **控制逻辑电路**:
  🤔我不太了解, 以我的认知, 我认为**里面是电路自动化的状态机**, 
  控制寄存器和状态寄存器会作为变量, 影响部分状态时的行为逻辑?
  .

- **时钟控制**:
  把[定时器](#timtimer定时器)吃懂, 这一块没啥问题, 勿复言尔.
  > 唯一一个特别的, 就是`控制逻辑电路`, 也就是 `硬件状态机`, 能够**影响时钟**

最后附一个基本结构图:
![](./images/2026-02-26-03-22-10.png)

#### 时序图
~本篇只谈论7位的情况，把7位的了解清楚，10位的也不在话下（应该？）~
##### 主发送器时序图
![](./images/2026-02-26-17-00-43.png)
> <span style = "color : red">规则怪谈:</span>
> 1. 这种时序图， 上面那行是关于 `SDA` / `SCL` 的相关通信操作；
> 2. 下面那行是关于 `事件和标志位` 的记录。
> 3.  **一个事件会产生多个标志位**.
> 4. `通信的操作`和`事件的标记`实质上都是, 对寄存器的某一位 / 某几位进行配置, 然后硬件状态机, 在不同时刻读取相应位, 做出相应决策.
> 5. `通信的操作`和`事件的标记`在某些特定`序列操作`或者某些特定`软件操作`后会被清除, 这些特定`序列操作`往往包含在状态机的执行中. ~具体哪些序列操作看上图~
  

- **S**
  由于上文提到, STM32的 I2C模块遵循的是可变多主机的模型, 使用前需要让该I2C模块从`从设备`变成`主设备`.
  所以要**配置CR1寄存器的START位**, 以产生起始条件， 此时状态机接到信息， 变为`起始状态`. 
  > 其实有点不严谨🤔, 在可变多主机中, 已经没有很清晰的主从概念了, 每个设备都有话语权, **配置CR1寄存器的START位**相当于夺取话语权的过程.

  .
- **EV5**
  在起始条件成功后, 置 `SR1` 的 `SB`位,  表明 起始条件成功发送， **状态机进入下一阶段**;
  > 这里把这个下一阶段姑且叫做`状态机 : SB`
  
  .
- **地址 + A**
  我们程序上读取该位，在软件层面确定起始条件成功发送，随后我们控制CPU写入数据寄存器, 此时**硬件自动清除`EV5`** .
  因为状态机在`EV5`时进入新阶段， 已经明白接下来要发送地址。
  我们向DR发送完数据, 状态机就知道该怎么做了.~包括发送8位数据且等待应答~
  .
- **EV6**
  数据发送结束, 且接收到`ACK`后, `SR1`的`ADDR` 置 `1`. ~接收到NACK不置一~
  
  `ADDR = 1` + `状态机 : SB` <span style = "color : red">→读SR1 , SR2→</span> `状态机 : ADDR` + `ADDR = 0`;
  .
- **EV8 / EV8_1 / EV8_2**
  🤔我问了AI, 观察了很久时序图, 我得出一个猜想:
  `EV8_1` == `EV8_2`, 是的, 他俩实质上一样, 都是`TXE` and `BTF` and `other Flag`的组合标志位群.
  `EV8`是 `TXE` and `other Flag`的标志位.
  .
  > `TDR`空 → `TXE` = 1; 
    `TDR`空 && `TSR`空 && 接收应答位 → `BTF` = 1;

  此时状态机会停止在当前状态, `SCL`也停留在高电平.
  如下是一些状态和标志位情况:~(按时间顺序排的)~
  |程序操作|状态|标志位情况|事件 / 时序|状态机下一步执行|
  |:---:|---|---|---|---|
  ||`TDR`空 && `TSR`空 && 接收应答位|`TXE` = 1, `BTF` = 1|`EV8_1`|暂停发送|
  |CPU → DR| 此时`TDR非空`, TSR非空 |`TXE` = 0, `BTF` = 0|数据上膛|空|
  ||此时`TDR非空`, TSR空|`TXE` = 0, `BTF` = 0|数据二重膛|TDR → TSR|
  || 此时`TDR空`, TSR非空|`TXE` = 1, `BTF` = 0|`EV8`|`TSR → SDA`|
  |CPU → DR| 此时`TDR非空`, TSR非空 |`TXE` = 0, `BTF` = 0|数据上膛|继续`TSR → SDA`|
  ||此时`TDR非空`, TSR空|`TXE` = 0, `BTF` = 0|数据二重膛|TDR → TSR|
  || 此时`TDR空`, TSR非空|`TXE` = 1, `BTF` = 0|`EV8`|`TSR → SDA`|
  |.........(重复上面三步操作)|||||
  || 此时`TDR空`, TSR非空|`TXE` = 1, `BTF` = 0|`EV8`|`TSR → SDA`|
  ||`TDR`空 && `TSR`空 && 接收应答位|`TXE` = 1, `BTF` = 1|`EV8_2`|暂停发送|
  |CPU → STOP|`TDR`空 && `TSR`空 && 接收应答位|`SR1 → STOP` = 1|P|发送停止条件|
  
  **状态机和程序的发送数据, 严格按照该时序执行.**
  - 那程序指令的发送还要注意时间?
    是的, 程序指令不能一股脑的发送, 需要**检测其前一条时序的事件发生后才能发送**.
    .
- **P**:
  触发停止条件, 让出话语权.
  **但不会复位自身寄存器配置**.
...
##### 主接收器时序图
![](./images/2026-02-26-20-46-37.png)
~与发送器一致的事件和操作就不讲了。~

- **EV6_1**:
  > 要先说一件事： 在硬件I2C中， 当主接收器时， **ACK是一个配置寄存器的其中一位**， 需要我们初始化时就设定好。
  这意味着， ACK的值是固定的， 如果初始化时是 ACK = 1, 最后结束接收时如果想要发出NACK， 需要手动操作， 让ACK = 0.
  
  好了, 回归正题, 聊聊主角:
  EV6_1 **不是一个硬件标志位，而是一个“软件操作窗口期”**。
  它的唯一作用是：
  当你只需要接收 1 个字节数据时，你必须在**接收该字节的这个极短时间窗口**内，同时**关闭 ACK 并生成 STOP 信号**。
  如果错过这个窗口（比如等收到数据后再操作），就会导致时序错误:
  1. 状态机仍然发送ACK, 从机可能会继续发送第 2 个字节，
  2. 或者 STOP 信号发送时机不对。
    .
- **EV7**:
  也就是老生常谈的`RXNE` = 1, 程序接收后马上读取数据, 并消除该事件. 
  .
- **EV7_1**:
  🤔怎么说, 其实就是`EV6_1`, 只不过不是第一个数据了而已. 
  `EV7` 和 `EV7_1` 的 区别只在于是否有在`软件操作窗口期`**关闭 ACK 并生成 STOP 信号**

### 软件 / 硬件 通信波形对比
![](./images/2026-02-26-21-07-50.png)
- **软件模拟**：
  1. 由于是手动加延时的原因, 有的时候加的多, 有的时候加的少, 所以每个时钟周期不是很规则.
  2. 也是由于手动延时的原因, 并没有在SCL下降沿产生的那一刻就进行SDA的读写操作.
  .
- **硬件实现**:
  1. 每个SCL时钟的周期 和 占空比都非常一致.
  2. 数据的读写紧贴SCL的边沿跳变.
   
但是I2C同步时序的特性, 可以容忍时钟的不规整 和 读写的延迟.
这就是同步时序的好处💪!!!
...
- **四根通信线**：
  1. SCK~（Serial_Clock）~
  2. MOSI~（Master_Output_Slave_Input）~
    代表主机的接收端 和 从机的发送端。
  3. MISO~（Master_Input_Slave_Output）~
    代表主机的发送端 和 从机的接收端。
  4. SS~（Slave_Select）~
      从机选择, 有多少个从机， 主机就连多少个SS线， 置低电平代表启动与从机的通信, 以代替I2C的多从机寻址问题.
  ![](./images/2026-03-02-15-29-10.png)
  > 这四种线代表四种功能, 具体名称可能在不同外设下不太一样.
  比如 SS, 又可以叫做CS, ~Chip_Select~.
  <span style = "color : red">注：</span>
  所有SPI设备的SCK、MOSI、MISO分别连在一起。
  <span style = "color : red">又注：</span>
  所有发送端引脚~输出引脚~, 都配置为**推挽输出**; ~因此高低电平都有很强的驱动能力，就可以实现更高的传输速度。~
  > > I2C为了实现**时钟延展** 和 **总线仲裁**, 也就是`线与`的设计哲学, 不得不使用`开漏输出`的模式.
  > 但SPI只要一时间**只连一个从机, 那么就不会出现一条线上出现两个输出的情况**，就不会出现冲突和电路短路的状况。
  >
  > 所有接收端引脚~输入引脚~, 都配置为**浮空或者上拉输入.**
  <span style = "color : red">又又注：</span>
  由于是单端信号，所以**所有的设备还需要共地**。
  - **从机的输出**：
    **只有当SS为低电平时， MISO才被使能变为推挽输出**，具体实现是依靠三态输出电路：~原理相同,细节上有所不同~
    ![](./images/2026-03-03-15-41-24.png)
    有如下规则怪谈():
    1. 图中的`E`引脚是SS接口；
    2. `IN`引脚是内部逻辑输出口;
    3. 上边的mos管是高电平导通, 下面的mos管是低电平导通.
    4. 上边的器件是`与门`, `有0则0`
        下边的器件是`或门`.`有1则1`.
    当`E`高电平时, 输出由`IN`决定;
    当`E`低电平时, 上端mos管是低电平, 下端mos管是高电平, **输出呈高阻态**. 
- **同步，全双工**
  `MOSI` `MISO`两条数据线在同时进行数据传输。
  其他勿复言尔.
  .
- **支持总线挂载多设备（一主多从）**
  **一般使用`一主多从`**， 比较少见多主机模式, 理由如下:
  ![](./images/2026-03-02-11-27-12.png)
  .
- I2C、SPI通信的**缺点 & 优点**:
  - **I2C**：
    - **优点**：
      ...
      .
    - **缺点：**
      1. **半双工**， 一个时刻只能发送或者接收， **发送和接收没协调好可能会产生冲突**。
      2. 通信线采取 **开漏输出** 和 **上拉输入** 的电路结构， 使得通信线高电平的驱动能力比较弱， 这导致了 **电平跳变时上升沿耗时较长**。
      
      这两点都**限制了I2C的最大通信速度**。
    > 总的来说，I2C可以在**消耗最低硬件资源**的情况下， 实现最多的功能， 主打一个性价比。
      
      .
  - SPI：
    - **优点**:
      1. 简单粗暴, 用法简单.
      2. 全双工， 不怕发送和接收没协调好，产生冲突。
      3. 速度极快, SPI协议没有严格规定最大传输速度, 实际使用时的最大传输速度取决于**芯片 / 外设厂商的设计需求**.~也就是说,协议没上限,得看芯片的负载~
      .
    - **缺点**:
      1. 实现的功能没有I2C那么多.
      2. SPI**硬件开销大**, 通信线个数也比较多, 而且经常伴随**资源浪费**的现象.
  
    > 总的来说, SPI比较壕无人性, 用`硬件开销`换取`操作简单`和`传输速度`.

#### 通信原理
如下的结构框图其实并不全面， 想看全面的结构框图， 可以跳转至[硬件SPI框图](#硬件spi框图)
![](./images/2026-03-03-16-11-07.png)
> 结构框图不全，更详细一点的应该如下：
![](./images/2026-03-03-18-44-15.png)

有以下规则怪谈():
1. 时钟信号的某一边沿, **主从机的移位寄存器都发送最高位数据到输出口**，同时向左移动一位,最低位补0；
  在另一边沿对输入口进行数据采样， 数据采集到最低位。
  在后续中，称第一个边沿为`移位边沿`， 第二个边沿为`采集边沿`。
  .
2. 移位寄存器的**最高位直接连接到一个 D 触发器**~（即锁存器）~，该触发器由 SCK 控制。在`采样边沿`后解除锁存, 在`移位边沿`前完成锁存.~个人认为是在下降沿时进行锁存,硬件状态机先锁存后移位.~
   .
3. 如果第一个边沿是`采样边沿`, 在`SS`置低电平后~`采样边沿`前, 会执行一次`锁存 + 移位`.~疑似，具体细节可能不太一样？~
...
```
- **具体过程**：
  在SPI通信通信中，按照上文的`规则怪谈`,主从机的移位寄存器同时进行工作, 也就是同时进行 发送 / 接收工作.
  当你只想进行**数据的发送时, 移位寄存器仍然会进行读取**, 但是只要我们不去进行读取, 就相当于没有(), 只不过是多点性能损耗, 管他的, SPI有钱任性()
  > 一般的, 在**只接收数据**的情况下, 一般向对方**发送0xFF / 0x00**;

#### 基本时序
- **交换一个字节**：
  **SPI每一个基本时序的操作实际都是交换一个字节**，
  所以先介绍一下交换一个字节的基本流程：
  ![](./images/2026-03-03-16-20-23.png)
  解释如下:
  ![](./images/2026-03-03-19-18-20.png)
  > <span style = "color : red">注：</span>
  > CPOL → Clock_Polarity   
  > CPHA → Clock_Phase
  ![](./images/2026-03-03-16-22-19.png)
  1.`移出数据`就是`移位边沿`, `移入数据`就是`采样边沿`.
  2.交换一个字节是最简单的SPI通信， **如果想继续发送，只要SS不拉高就行**。
  3.CPHA在程序中实际上选择的是`采样边沿`是奇数边沿还是偶数边沿.

- **时序组成**：
  - **I2C通信**:
    我们先温习一下I2C通信的[时序集成](#时序集成)：
    以`指定地址写`为例:
    1. 发送从机地址, 唤醒从机.
    2. 发送寄存器地址, 实现指定地址写.
    3. 发送要写入的数据, 实现写入.
  ...
  - **SPI通信**:
    ...
    如下是操作手册的指令集表:
      ![](./images/2026-03-03-20-14-24.png)
      > <span style = "color : red">注:</span> 
      `字节1`是指令, ~h是代表前面的数是16进制~.
      后续的字节安排也已经表明, **无括号表示发送**, **有括号代表接收**, ~但页编程和四路页编程的括号不是~。
      `dummy`是发送`无意义数据`~一般发送0xFF~, **主从机只发送都不接收**, 其目的在于**消耗时间**,等待从机处理好数据.

    可以见得, SPI更像是**面向标准库操作**.
    .
- **一次通信传输一条指令**：
  在SPI中，起始条件和终止条件意味着的是**指令的起始和终止**；
  每个指令结束就要发送通信的终止信号。原因是上文所说的，**指令相当于库函数**，一个指令要做事情可能很多，**完成一个指令后，从机需要一定时间消化**， 此时就会置`busy`标志位.
  **这时只有部分读取寄存器之类的指令能够被从机反应**, 这时我们就要**循环读取**`读取CR1的指令`, 直到读取出的`BUSY`标志位**置0**为止; 此时可以发送下一条指令.
  举个例子——`指定地址写`
  ```text
  写使能指令
  读取CR1指令
  ...
  ```
  > tips:
  **指令是与从机的指令**, 指令的数据写入要求和操作规范都是从机的产品说明书决定的.~包括指定地址读写时的地址位数.~
  其实归纳起来, **所有通信协议, 主机都只掌握话语权, 但是对通信的理解是取决于从机的**.

### SPI外设模块
#### 简介
该硬件可以自动执行**时钟生成**、**数据收发**等等，减轻CPU负担。以下是该硬件的一些功能特点：
- 可配置**8位/16位**数据帧、**高位先行/低位先行**
  最常用配置是 `8位数据帧 + 高位先行`.
  16位数据帧, 顾名思义, 就是`DR`一口气收发16位. 这一功能的实现得益于SPI外设模块的**DR是16位的寄存器.**
  .
...
- **兼容I2S协议**
  ~不是I2C!!!~
  I2S是一种**数字音频信号传输**的专用**协议**
  因为有些电路的结构相似, 所以就做一块了.
  ![](./images/2026-03-17-14-23-22.png)
  .
> STM32F103C8T6 硬件SPI资源：SPI1(挂载于APB2)、SPI2(挂载于APB1)
> <span style = "color : red">注:</span>
> 如上所述，STM32内置的SPI外设模块功能十分多元且强大，这是因为硬件电路一旦设计，则功能钉死，只能通过一些寄存器来改变相关配置。
> 但，这也造成**学习曲线陡峭**的问题。
> 因此，在下文中只介绍最简单常用的主线知识点。
> 至于剩余的进阶方法，等以后实践了，大家 `问AI + 查手厕儿` 慢慢摸索罢（

#### 硬件SPI框图
![](./images/2026-03-03-17-04-56.png)
与前文的串口和I2C通信的结构总体上差不多, 这里分为两个部分——`收发数据模块`和`状态配置模块`。
- **收发数据模块**
  这里的结构和**串口的收发分离**, **I2C的收发共用**, 都有些不同, 也可以说是中间体——SPI共用一个移位寄存器，但是他们的收发缓存区~（串口、I2C的数据寄存器）~是分离的。
  > 但是收发缓存区在程序层面还是共同为一个寄存器`DR`.

  这种结构非常好的实现了，**一时钟周期内**就能有序**完成发送和读取**的操作。
  以下讲一些具体细节:
  - **LASTFIRST控制位**:
    这一位可以控制低位先行还是高位先行.
    .
  - `MOSI` / `MISO`:
    这里集中讲一下方框里将`MOSI` / `MISO`作交叉的那两条箭头:
    其实就是**切换主从模式用的**, 当切换为主模式时不用, 当切换为从模式时开启.
    
  .
- **状态配置模块**
  大体内容勿复言尔....下面讲一些具体的功能配置:
  - **波特率发生器**:
    产生分频后的时钟周期, 随后引到`SCK引脚`和`SPI内部通信电路`.
    分频率由`SPI_CR1`的`BR0 / BR1 / BR2`决定.
    .
  - **NSS(从机选择)**:
    `N`~negative~, 负的. 所以是**低电平有效**.
    `NSS`, 是一种**抑制主机选择**. 所有设备的NSS线连在一起, 
    从机默认是接收模式, `NSS`作为接收端, 接收到低电平时, 无法升格为主机.
    当有一个擢升为主机时, `NSS`就会变为发送端, **发送低电平, 抑制其他设备变为主机**.  
    > 这里的 `NSS` 和 之前提及的 `SS`实现两种功能, 不是同一个东西.
     
    .
    - **配置方面**:
      1. 输出时:
        作为主机时, `SPI_CR2`的`SSOE`会置1, `NSS`此时作为输出引脚. 当主机模式结束后,要清零`SSOE`.
        .
      2. 输入时:
        ![](./images/2026-03-17-15-22-46.png)
        `SSM`控制多路选择器:
        选0路时, 接收外来的NSS信号;
        选1路时, 由芯片内部的`SSI`决定是否为低电平.   
        .
    - 记得开`SPI_cmd`()
...
##### 全双工连续传输时序图
~下图以上升沿移入,下降沿移出的模式为例~
![](./images/2026-03-17-15-42-43.png) 
该图的输入输出应该需要都退后一个时钟周期, 比如`b0 → b1 .... b7 → next b0`
原理其实没啥好说的, 无非就是:
1. 移位寄存器发送为空, 则吃一口发送缓存区.
    .
2. 移位寄存器接收为满, 则喂一口接收缓存区, 随后自己饿了, 吃一口发送缓存区....
    .
3. TXE下降沿 → 总线填入缓存区;
    上升沿 → 移位寄存器吃一口.
    .
4. RXNE下降沿 → 移位寄存器喂一口缓存区;
   上升沿 → 总线吃一口缓存区.
    .

##### 全双工非连续传输时序图
![](./images/2026-03-17-16-01-57.png)
该图的输入输出应该也需要都退后一个时钟周期, 比如`b0 → b1 .... b7 → next b0`
和`全双工非连续传输时序图`没啥区别, 唯一的区别就是:
~叠甲，以下叙述我其实没搞明白，似乎如此，大家看看就好~
**连续的**时序为: 
```text
移位寄存器→RDR 👉 WDR→移位寄存器 👉 RDR→总线 👉 总线→WDR
移位寄存器没闲过
```

**非连续的**时序为: 
```text
移位寄存器→RDR 👉 RDR→总线 👉 WDR→移位寄存器 👉 总线→WDR
移位寄存器闲过，闲时BSY位置0，SCK停止跳变
```
其中， **这也就导致了`BSY`位和`SCK`的变化也有所不同**
由于这个`闲时空窗期`的时间大小固定, 所以**非连续性传输在高频率传输时空窗期占比变大， 在传输速度上拖的后腿越来越大。**
- 那非连续性传输有什么好处？
  🤔， 代码简单，四行代替大脑🤓👆

> 剩余部分有疑问可以移步转问 `AI 和 手厕儿`
...
如果 IN+ <一般的输入口是高电平有效, IN- → 输出 4号引脚(接GND)（内部晶体管导通到 GND）
...
- 缺点
  如上述所言， `当MPU6050静止时，加速度计测量的是重力加速度在各轴的分量：`, 所以物体在运动时, 测量会产生误差.
  也就是具有静态稳定性, 但不具有动态稳定性.
...
- **缺点**:
  1. 虽然它们设计用于测量角速度, 但**线性加速度也会对振动质量施加力**, 这就导致**系统遇到大的加速度, 精确性会受到影响**.
  2. **漂移** 。陀螺仪存在零偏误差，即使**设备静止**，它也会**输出微小的非零值**。随着时间推移，积分误差会无限累积, 测量不准确。
      也就是**具有动态稳定性， 但不具有静态稳定性**。
- 角速度和姿态角有什么关系?
  **角速度积分后就是姿态角()**
###### 光学陀螺仪
|||
...
##### 数据融合
加速度计具有静态稳定性, 但不具有动态稳定性;
科里奥利效应振动传感器具有动态稳定性， 但不具有静态稳定性.
我们只要静态时测量`加速度计` 的值, 运动的时候测量`科里奥利效应振动传感器` 的值, 就可以做到测量姿态角了!
所以这个组合就皆大欢喜了！！！ 
**....可是, 当真如此吗?**
真正的数据融合算法与上述所言, 相差甚远, 实际上是一个连续的、实时的反馈过程, 如下:
陀螺仪以极高的频率~（如1kHz）~输出角速度,并进行积分累加。
加速度计和磁力计也以较高频率~（如100Hz-200Hz）~输出**绝对角度参考**。
算法~（如互补滤波、卡尔曼滤波）~会在每一个计算周期里，都会把**陀螺仪推算的角度**和**加速度计/磁力计测量的角度**根据当前运动状态进行**加权融合**。
- 这里的**根据当前运动状态**是什么意思呢?
  **加速度计**：
  在设备运动时（只要有重力分量且线性加速度不大），它依然在工作并修正陀螺仪。
  只有在剧烈加速（如猛推一下）时，算法才会暂时“不信”加速度计，主要依赖陀螺仪。
  .
  **磁力计**：
  无论设备怎么转动，**只要磁场环境稳定**，它一直在提供绝对方向参考。
  .
- 有加速度计和陀螺仪不是就够了, 磁力传感器 和 气压传感器的作用是什么呢?
  原因如下:
  由于加速度计的原理依靠重力, 这会给其带来限制——**加速度计只能测量当前的`绝对俯仰`和`绝对滚转`.**
  但陀螺仪只能测量当前一小段时间的`俯仰 / 滚转 / 偏航增量`
  .
  所以就需要一个传感器来测量`绝对偏航`, 那就是**磁力计**. 
  只要周围没有强磁干扰，地磁场永远指向磁北。算法利用这一点，强行把陀螺仪算出来的朝向角拉回到磁北参考系.
  .
  而气压传感器的作用是确定绝对海拔高度, 原理不必言明.

#### 硬件参数
~终于到整体硬件了😭~
**16位ADC采集传感器**的模拟信号，量化范围：-32768~32767
> 这意味着, 每个传感器的数据寄存器都有两个.

加速度计满量程选择：±2、±4、±8、±16（g）
陀螺仪满量程选择： ±250、±500、±1000、±2000（°/sec）
> 由配置寄存器的两位控制, 刚好四种情况.
> 量程越小, 分辨率越高; 量程越大🤔, 量程越大🤣👆

可配置的数字低通滤波器;
可配置的时钟源;
可配置的采样分频;
> 虽然I2C是同步时钟, 但是那是发送 / 接收数据的时钟;
> MPU6050的内部逻辑电路是有自己的一套时钟.

I2C从机地址：1101000（AD0=0）/ 1101001（AD0=1）
> 地址有两种切割方法, 一个是直接转成十六进制, 即 110 1000 → 0x68
> 根据I2C发送时序, 发送时地址在高七位, 所以还有一种是左移一位, 即 1101 0000 → 0xD0. 这样发送地址的时候就不用刻意再左移一次了.

#### 硬件电路
|||
|---|---|
|![](./images/2026-02-25-18-10-43.png)|| 
|![](./images/2026-02-25-18-10-25.png)||
|左下角的这个是芯片的外接口<br>左上角是`LEO`也就是一个`低压差线性稳压器`|右边是内部的`MPU6050核心芯片`|

我们接下来一一分析下:
- **MPU6050核心芯片**:
  看图可见, 该芯片的引脚很多, 但有很多引脚的连接需求是PCB已经完成的, 属于该MPU6050外设内部的封装细节.只余8个外接口需要我们进行完善.
  但我们先不管外接口， 我们先看一些`MPU6050核心芯片`的引脚或者模块:
  - `SDA` / `SCL`
    内部已上拉3.3V, 外部无需再上拉了.
    .
  - `DMP`
    芯片内部有`DMP`单元, 进行传感器数据融合与姿态解算.
  - `XDA` / `XCL`
    - **主机模式**
      此时, **MPU6050 的内部 I2C 主机控制器被启用。**
      这时 `MPU6050`能够作为主机，外接其他从机~比如其他传感器~, 然后把相应传感器信息一起存储在MPU6050的数据寄存器中， 然后一起进行`DMP单元`的数据融合和姿态解算.
      再偷个懒():
      ![](./images/2026-02-25-19-17-02.png)
      优点的意思就是, MCU软件资源消耗低, 电路也简单, **时钟同步, 一次性获取所有传感器信息**.
      > 这里很有意思的点是, 如果使用了`主机模式`, 那么它就只受MPU6050的固件逻辑控制, 而不受我们的MCU控制;
      什么`我的附庸的附庸不是我的附庸`()

      .
    - **旁路模式**
      此时, **MPU6050 内部将 SDA/SCL 与 XDA/XCL 直通。**
      再再偷个懒():
      ![](./images/2026-02-25-19-20-09.png)
      > 流程的意思就是, 把其他传感器连在MPU6050的`XDA` / `XCL`上, 这时相当于MCU通过`SDA` / `SCL`也连到了其他传感器上.
...
- **外接口**:
  ...
  - **AD0**:
    [硬件参数](#硬件参数)里有提到一点,
    控制从机地址的最低位, 
    AD0 接高电平则从机地址 110 1001;
    低电平的话就是 110 1000.
    > 看电路图, MPU6050内部已经默认该引脚弱下拉了.

    .
  - **INT**:
    内部配置好中断事件后, 可以通过这个引脚向MCU发送中断信号, 
    同时**MCU要开启相应通路的`EXTI`**.
    .
- **LDO**:
  ![](./images/2026-02-25-19-35-13.png)
  MPU6050核心芯片的VDD供电是2.375-3.46V;
  为了把整个模块的**供电范围提升到5V**, 所以加了个**3.3V的稳压器**, 也就是`LDO`.
  左边的`VCC 5V`是输入端电压， 右边的`VCC 3.3V`是输出电压;
  `VCC 5V`连接模块外接口的VCC, 供电范围在3.3V - 5V ;
  然后经过稳压器, 得到稳定的3.3V电压.

#### 模块框图
如下是MPU6050**核心芯片**的框图：
![](./images/2026-02-25-19-38-45.png)
- `CLKIN` / `CLKOUT`
  用于选择外部时钟, 以作为内部逻辑电路的时钟.
  > 而且在我们的这个MPU6050模块的[硬件电路](#硬件电路-1)中, 
  `CLKIN`接地, `CLKOUT`直接没有引出, 所以这部分在我们这个MPU6050模块里没用.
  .
- **传感器元件**
  Accel / Gyro / Temp Sensor 都是传感器元件, 其实**这些传感器元件的本质都是分压器**, 之后通过`ADC`, 把输出的模拟电压转变成数值, 放到数据寄存器内. 随后通过`DMP`得到相应值.
  > 这里的操作都是电路自动进行的, 我们只要**设定数据刷新的频率就行**. 
  每个传感器都有自己的两个8位数据寄存器~因为这个模块的ADC是16位的~ 

  .
  - **自测单元**:
    目的是测量传感器模块是否故障.
    当启动自测后, 芯片内部会施加一个静电力的激励信号，使机械结构产生微小振动, 
    以**模拟一个外力施加在传感器上, 这个外力会导致传感器数据会比平时大一些**. 具体自测流程如下:
    1. 先使能自测, 读取数据, 放在相应数据传感器里.
    2. 再失能自测, 读取数据, 放在相应数据传感器里.
    3. 两个数据相减,得到一个新数据, 叫`自测响应`, 放在相应数据传感器里.
    4. 判断自测响应大小, 如果自测响应在一定范围内, 那么传感器模块就没有出现问题; 
    如果不在, 就意味着传感器模块或者其他模块出现故障了,  需要悠着点用.

    > <span style = "color : red">注:
    自测响应需要MCU自行读取, 然后参考 MPU6050 内部的工厂校准数据, 自行判断其数据是否正常</span>

- **电荷泵**:
  也叫`充电泵`.
  具体原理可看这个视频[【电荷泵是如何升压的？原理非常简单，一看就会！】](https://www.bilibili.com/video/BV1xX4y1F7Lp?vd_source=7acd15f8dc409504a76f1086442daa5a)
  ![](./images/2026-02-25-22-13-11.png)
  我这里谈一点观后感：
  **真的很像泵耶！！！**
  以下是规则怪谈时间：
  1. 利用正极的5V电压提供源源不断的**活水**， 
  当C1的上极板电压小于5V时， 就会有电流从正极流过来。
  .
  2. 左边的脉冲信号~应该是时钟信号?~, 反复在`GND ~ VCC`中间变换, 
  **由于变换时, 电容上下极板的电压差不变**.
  升压时, C1上极板的电压会随之增加5V; 降压时, C1 上极板的电压会随之降低5V. 
    .
  3. 二极管在这里就像一个水流的单向阀, 只进不出.
    .
  4. 输出C0通向电子元件, 进行直接的供电.
    .
  在这种反复的"抽水"过程后, C0上极板的电压理想情况下能来到 `2 * VCC`.
  ![](./images/2026-02-25-22-40-59.png)
  .
- **中断状态寄存器**:
  控制内部哪些事件可以开启和中断引脚的连接.
  .
- **FIFO先入先出寄存器**:
  说白了就是栈, 一个相当于缓存区的地方.
  .
...
- **工厂校准**:
  管`自测单元`那一块的,
  当你开启自测时，MPU6050 内部会施加一个固定的静电力，让传感器的机械结构产生位移。**这个位移量在理想情况下是固定的物理量**。
  但有个问题：
  由于 MEMS 制造工艺的微小差异，**每一颗芯片的机械结构灵敏度都不完全一样**。同样的静电力，芯片 A 可能产生 1000 LSB 的输出变化，芯片 B 可能产生 1200 LSB。
  所以后果：
  如果你用一个固定的标准（比如“必须等于 1100”）去衡量所有芯片，那么很多正常的芯片会被误判为故障。
  > 为了解决上述问题，InvenSense 在出厂前会对每一颗 MPU6050 进行单独测试。
  他们运行自测程序。
  测量该特定芯片在自测模式下的实际响应值。
  **将这个特定的响应代码写入芯片内部的寄存器**（SELF_TEST_X, SELF_TEST_Y, SELF_TEST_Z, SELF_TEST_A）。
  这些数据是**只读**的，永久保存在`工厂校验`的指定寄存器里。

  所以我们执行自检的时候, 不仅要观察`自测响应`, 还要读取`工厂校验`的指定寄存器里的工厂码.
  .
- **FSYNC 帧同步**
  它的目的是确保发送端和接收端在“时间”和“内容边界”上保持一致，
  让接收方知道哪里是一帧数据的开始，哪里是结束，或者确保多方在同一时刻处理同一帧数据。
  偷个懒（）：
  ![](./images/2026-02-25-22-58-10.png)
  .
- 其他地方在上文的某一处讲过了, 此处不赘述了.
#### 琐碎的补充
以下内容是看手厕儿的， 怎么看手册就不讲了。
- 该芯片的SCL时钟频率最大只可达400KHz。
  超过该频率， 芯片就跟不上了。
  .
...
- **时钟源选择**
  生成内部时钟的允许**内部源**包括：
  1. 内部的 RC振荡器
  2. X、Y或Z陀螺仪中的任何一个（微机电系统（MEMS）振荡器, 这个时钟同样用于产生陀螺仪内部小物块的振荡运动）
    > 不使用低通滤波器`DLPF`时, 陀螺仪时钟为8KHz;
    使用后为1KHz.
    `DLPF`可以让数据更平滑, 配置滤波器的参数越大, 输出数据抖动越小.
    .
...
- **中断配置**
  兄弟你自己看手册吧,今天手写痛了😭;
  > 注意一下, 运动检测中有一个`高通滤波器`, 隔直同交.
  用于滤除重力造成的稳定数据偏移, 也就是滤除加速度计静置时那一个g的重力加速度.

- 传感器数据寄存器里的值**已经是16位的有符号整数**了, 以二进制补码方式存储.
  .
- 从`WHO_AM_I`读取地址时, 寄存器只发送高七位的地址
  也就是得到 110 1000 → 0x68;
  > 其中, 这个寄存器是只读的, 其内容不随AD0引脚的配置变化.
  始终都是 0x68 .

  ![](./images/2026-02-25-23-44-50.png)
  .
- 所有寄存器 上电 / 复位 后默认都是0x00;
  > 除了`电源管理寄存器1`, 和`我是谁寄存器`
  其中, `电源管理寄存器1`是0x40, 开启了睡眠模式 , 在初始化的时候需要解除;

### W25Q64
#### 简介
![](./images/2026-03-04-11-05-10.png)
`W25Qxx`系列是一种低成本、小型化、使用简单的**非易失性存储器**，利用SPI进行通信, 常应用于数据存储、字库存储、固件程序存储等场景
> `固件程序存储`是把程序文件直接放在外挂芯片中, 需要执行时, 直接读取外挂芯片, 这就是XIP~(eXecute_In_Place)~

| 特性 | 传统方式 (Copy to RAM) | 就地执行 (XIP) |
| :--- | :--- | :--- |
| 流程 | 上电 -> Bootloader 将代码从 Flash 复制到 RAM -> 跳转到 RAM 执行 | 上电 -> 处理器配置好总线 -> 直接从 Flash 取指执行 |
| RAM 需求 | 高：需要足够的 RAM 来存放所有运行的代码和数据 | 低：RAM 仅用于存放变量、堆栈和动态数据 |
| 启动速度 | 慢：复制代码需要时间（尤其是代码量大时） | 快：省去了复制过程，几乎立即开始执行 |
| 执行速度 | 快：RAM 的读写速度通常远快于 Flash | 较慢：受限于 Flash 的读取速度和总线带宽 |
| 成本 | 较高（需要大容量 RAM） | 较低（可以选用小容量 RAM 甚至无外部 RAM） |

- **存储介质**: Nor Flash(闪存)
  .
- **时钟频率**：80MHz / 160MHz (Dual SPI) / 320MHz (Quad SPI)
> `Dual SPI`双路SPI, 此时`MOSI`和`MISO`都用来**同时**的发送或者接收, 虽然每根线的最快频率是80MHz, 但是加起来就有160MHz了();
> 此时数据速率~（也就是带宽）~翻倍了，但移位寄存器的时钟频率并没有翻倍. 也就是**位宽加倍，频率不变**原理如下:
> ![](./images/2026-03-04-11-27-30.png)
> ![](./images/2026-03-04-11-27-52.png)
> 同理, `Quad SPI`四路SPI, 此时`MOSI` / `MISO` / `WP` / `HOLD` 都用来**同时**的发送或者接收.至于`WP` / `HOLD`是什么, 后续会介绍.
...
- **存储容量**(24位内存地址):
  ...
  <span style="color:red">注:</span>
  1. `W25Qxx`的`xx`就是存储容量为`xxMbit`的意思,~`W25Q40`/`W25Q80`反过来了,两位数要倒着看~
  2. 由于芯片内部采用24位的内存寻址, 所以最大寻址空间是16MByte, 这导致`W25Q256`无法被完全寻址, 所以`W25Q256`采用的是 可选的24 / 32位的内存寻址.

#### 硬件电路
![](./images/2026-03-04-11-44-53.png)
> 老规矩：左边是外设引脚， 右边是核心芯片引脚。
> 然后右边的二极管`D1`是电源指示灯.
>   <span style="color:red">这里有一点:</span>
 一般的输入口是高电平有效, 但是**如果某一个引脚的头上有横线, 或者前面有横杠, 那就是低电平有效**.比如上面的`头上戴横杠的CS`或者写成 `/CS`

由于时间关系, 就不一一介绍了, 下面讲一些[SPI-串行外设接口](#spi-串行外设接口)里没讲的东西:
- **WP:**
  `写保护`, 低电平有效.
  它的工作原理是**通过内部逻辑电路与状态寄存器中的控制位协同工作来控制写保护功能**。
  当 WP 引脚被拉低时，它会强制芯片进入硬件写保护状态~（前提是状态寄存器中的~ ~SRP~ ~位配置允许硬件控制）~。
  当 WP 引脚被拉高或悬空~（内部有上拉）~时，硬件写保护功能通常被禁用，写保护状态由软件~（状态寄存器中的~ ~BP~ ~位）~控制。
  .
- **HOLD**:
  `数据保持`, 低电平有效.
  用于**暂停 SPI 通信序列, 芯片时序不会终止, 会记住当前的指令状态**
  - **操作方式：**
  当 CS 为低电平~（即芯片被选中）~时，如果将 **HOLD 拉低**，**芯片会立即暂停当前的操作**~（时钟~ ~SCK~ ~和~ ~数据~ ~IO~ ~被忽略）。~这样从机就`hold`住了()🥶
  当 HOLD 再次拉高时，**通信从暂停处继续。**
  - **目的：**
    主要用于在多主设备系统中，让高优先级的设备可以暂时“插队”使用总线，而**不必重新发送整个命令序列**。
    或者, **主机进入中断**时, 从机不会在主机**没空改变发送和完成接收**时, 从机一股脑执行正在运行的指令, **导致指令结果出错**

#### 模块框图
![](./images/2026-03-05-19-46-14.png)
- **存储器**：
  对存储器的详细介绍可以去看看`计算机组成原理`的相关内容, 比如[【计算机组成原理（哈工大刘宏伟）135讲（全）高清】](#https://www.bilibili.com/video/BV1t4411e7LH?p=24&vd_source=7acd15f8dc409504a76f1086442daa5a)。 
  > 现在先理解**存储器以字节为单位，每个字节被分配唯一的地址**。
    上图中的`规划示意图`的字节地址表示是  
    从左到右自增，**每行`100h`个字节，~也就是256个字节~**
    从下到上自增，一共 `800000h / 100h = `8000列。

  - 地址划分
    存储器的容量一般很大，如果对存储空间进行划分，直接按一整块来使用，**那进行写入和读取时的寻址比较麻烦**， 并且涉及到`Flash擦除`时不好处理。
    所以，为了解决这种寻址问题，我们需要有一个寻址的基本单元，所以我们进行以下的存储空间划分：~以W25Q64的8MB超大杯存储空间举例（）~
    ```TEXT
    一整块存储空间  >  块 Block  >  扇区 Sector  >  页 Page
         8MB            64KB            4KB         256B  
                                                    👇    
                                       这就是规范示意图里的一行            
    ...
    .
    - **Flash擦除**
      Flash擦除, 是以 `页 / 扇区 / 块` 为单位的, 擦除时对应单元的所有字节都会被擦除.
      > **不是任一连续的256B的存储空间都可以叫做页**, 只有0xXXXX00-0xXXXXFF才能叫做`第XXXX页`, ~扇区和块类似~ 
  - 非易失性
    在图片的中下方, 有一个`高电压生成器`, 为`浮栅晶体管`的`量子隧穿`提供高电压.
    这导致了**存储器中的数据从 1 到 0 易, 但从 0 到 1 难**, 
    **同时数据写入的速度也会变慢**
    具体原理可看[【用最好的动画为你讲解--固态硬盘(SSD)的原理】](https://www.bilibili.com/video/BV1jDgtzEEVe?vd_source=7acd15f8dc409504a76f1086442daa5a)
    固态硬盘和Flash闪存的非易失性原理一致.
    > 🤔,确切来说, `Flash`是技术也是存储器, 而`SSD`是一个完整的设备, 内含Flash.

    | 特性 | Flash (闪存) | SSD (固态硬盘) |
    | :--- | :--- | :--- |
    | 定义 | 一种半导体**存储介质技术**。 | 一种存储**设备产品**。 |
    | 组成 | 基本的存储单元（Cell），组成页（Page）、块（Block）。 | 由多个 NAND Flash 芯片 + 主控控制器 + 缓存(DRAM) + 接口电路组成。 |
    | 角色 | 数据的实际“仓库”。 | 管理仓库的“智能管家” + 仓库本身。 |
    | 非易失性来源 | 直接来源于浮栅晶体管的物理结构。 | 来源于其内部集成的 Flash 芯片。 |


- **逻辑与执行模块**
  芯片内部自动化的关键，根据寄存器的配置和状态，进行`地址锁存`，`数据读写`等操作。
  - **地址锁存 / 计数器**
    分为`页地址`和`字节地址`. 回顾上文, 举个例子, 假设我现在要求读写的地址为`0xabcdef`, 一页是`0x0000FF`个字节, 
    所以页地址就是`0xabcd`, 页内的字节地址就是`0x00ef`.
    可见, **我们发送的24位地址应该前两个字节进入页地址锁存计数器内, 后一个字节进入字节地址锁存计数器内**.
    .
  - **页缓存区**
    硬件上其实是RAM存储器, 用于快速发送 / 接收数据.
    - 有了Flash这个存储器了, 为什么还要多此一举的再来一个缓存区呢?
      **Flash因为非易失性这个特性, 数据的写入速度会略逊一筹, 跟不上SPI通信的节奏.** 但是缓存区, 类似内存`DDR`, 是易失性的, 写入的速度就很快.
      所以需要缓存区来进行快速缓存.
      > 非易失性只会影响写入速度, 不影响读取速度.

    皆大欢喜, 我们完美理解了这个设计!!!
    可是....但真如此吗?...
    还有以下4个细节:
    ~读取指令没有以下事项~
      1. 由于页缓存区也才256字节, 所以一个写入指令, 一次最多也只能写入256字节, **那如果超过, 会覆盖原先的数据**.
      2. 如果起始写入的字节是0xAD, 那**写入后指针会向后移一位**, **直到写入0xFF时, 指针会移到0x00**.然后继续写到0xAC.如果再写, 就会覆写0xAD的数据.
      3. 指令结束后把缓存区数据一整块放置到对应页地址的区域内.~0x00对应0x00~
      4. 指令结束后, 芯片忙于转移数据, 会挂起`busy`标志位.
        .
    
    总结以下, 会有以下操作注意事项:
    写入操作时：
      1. 写入操作前，必须**先进行写使能**
      2. 每个数据位**只能由1改写为0，不能由0改写为1**
      3. 写入数据前必须先擦除，**擦除后，所有数据位变为1**
      4. 擦除必须**按最小擦除单元进行**, 且**擦除后也会进入忙状态**.
      5. 连续写入多字节时，**最多写入一页的数据，超过页尾位置的数据，会回到页首覆盖写入**
      6. 写入操作**结束后，芯片进入忙状态，不响应新的读写操作**
      .
    > 写操作的不便携其实是对成本的妥协()
    
    读取操作时：
      1. **直接调用**读取时序，无需使能，无需额外操作，**没有页的限制**
      2. 读取操作结束后**不会进入忙状态，但不能在忙状态时读取**

#### 手册信息抓取
想看寄存器 配置逻辑 和 对应意义的 👉 Registers
想看指令集 及其后续时序的 👉 Instructions
> 有一个指令是查看厂商和设备ID, 可以在SPI通信开始时执行, **目的是观察通信是否出错**.
> 部分重要指令会详细介绍时序图.

想看电气特性, 即`元件物理量的可接受范围` 和 `指令执行时间`的 👉 ELECTRICAL CHARACTERISTICS  