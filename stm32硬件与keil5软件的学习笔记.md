...
- **发送移位寄存器**
  ...
  - 那发送移位寄存器怎么知道什么时候是起始位, 什么时候是停止位, 什么时候进行移位呢?
    硬件状态机永远的神, 只能说是.
    .
  - 什么时候TDR转移数据到`发送移位寄存器`里?
    当 TDR 被写入数据的时候, 硬件自动化检查, 当前移位寄存器是否有数据正在移位~应该是通过查看状态机~
    如果没有, TDR数据就会马上发送到`发送移位寄存器`的相应位置, 同时置一个标志位, 叫`TXE`~(Tx_empty)~; 这个标志位置 `1` 后就会在TDR中写入下一个数据.
    > <span style="color : red">注: 1. 此时的发送移位寄存器还没完全向外发送完
      2.TDR被写入数据后会自动清零 `TXE` 标志位</span>
