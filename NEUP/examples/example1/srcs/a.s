...
.LFB86:;函数栈帧开始的位置标记, 告诉调试器,这个函数栈帧的开始
	...
	.seh_setframe	%rbp, 0
	subq	$48, %rsp; 将rsp地址减去48, 开辟 48 字节的栈空间。这是为了存放局部变量（如 i, length）,地址对齐 和 调用其他函数时需要预留的阴影空间
	.seh_stackalloc	48
	.seh_endprologue
	movq	%rcx, 16(%rbp); 把rcx寄存器上的值存到 RBP 向上 16 字节 的栈位置上。% rcx 里存的是函数的第一个参数
	;以上是函数的准备工作, 和下面是函数功能的实现
	...
	movl	%eax, -8(%rbp);值存在 -8 ~ -1 这 8 个地址
	movl	-8(%rbp), %eax
	addl	$1, %eax
	cltq; 把 eax 里的 32 位整数，“补全” 成 64 位整数，放到 rax 里。说白了, 就是int → long long的转变
	movq	%rax, %rcx
	call	malloc; 返回值放在rax里
	movq	%rax, %rdx
	movq	16(%rbp), %rax; 把ptarget指针的地址传到 rax 里
	movq	%rdx, (%rax); 把rdx里刚 malloc 的首地址传到ptarget指针的地址里, 因为有加(),所以不是rax寄存器本身
